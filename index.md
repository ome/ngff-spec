...
The `path` MUST be case-sensitive, and MUST NOT be a duplicate of any other `path` in the `images` list. The `path` MUST follow [Zarr node name naming conventions](https://github.com/zarr-developers/zarr-specs/blob/main/docs/v3/core/index.rst#node-names). The `path` MUST NOT only be `.` or `..` or start with the reserved prefix `__`. The `path` MUST NOT be an empty string and MUST NOT contain `/` characters.
Not empty, no /. The `path` SHOULD only use characters in the sets `a-z`, `A-Z`, `0-9`, `-`, `_`, `.`. 
